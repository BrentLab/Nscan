...
BEGIN{
	if($ENV{TWINSCAN}){
		$lib ="$ENV{TWINSCAN}/lib";
	}else{
		print "\nERROR: please set the Twinscan environment variable, eg\n\nexport TWINSCAN=\"/home/bart/Twinscan\"\n\n";
		$lib=$0;
		$lib =~ s/conseq.pl/..\/lib/;	# in the distribution, the lib directory is parallel to the bin directory that contains conseq
		exit();
	}
}
use lib $lib;
