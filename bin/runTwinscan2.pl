...
my ($opt_d, $opt_blat, $opt_B, $opt_species, $opt_r, $opt_x, $opt_v);
GetOptions(
	   "d=s" => \$opt_d,
           "B=s" => \$opt_B,
	   "species=s" => \$opt_species,
           "blat=s" => \$opt_blat,
	   "r=s" => \$opt_r, 
	   "x" => \$opt_x,
	   "v" => \$opt_v);
...
REPEATMASKER OPTION:
-species    Species can be human, mouse, rattus, mammal, carnivore, rodentia, rat, cow, pig, cat, dog, chicken, fugu,
...
Mandatory Files (will be created if they don't exist):
  <target>.{masked, blast, conseq, twinscan, gtf}
";
...
#  Parameters
my $REPEATSPECIES = $opt_species ? $opt_species : "human";
my $RUN_XDFORMAT     = $opt_x;
my $TWINSCAN_PARAM   = $opt_r ? $opt_r : "$TWINSCAN_PATH/parameters/worm_twinscan_est.zhmm";
my $BLAST_PARAM      = $opt_B ? $opt_B : "$TWINSCAN_PATH/parameters/blast_params/Celegans.blast.param";
...
open(PROGRESS,">$OUTPUT_PATH/$TARGET.twinscan.progress") or die "could not open $OUTPUT_PATH/$TARGET.twinscan.progress\n";
...
if (-e "$OUTPUT_PATH/$TARGET_FILE.masked") {
	...
	system "$REPEATMASKER -species $REPEATSPECIES -dir $OUTPUT_PATH -nolow $TARGET";	# only mask interspersed
        system "if [ ! -f $OUTPUT_PATH/$TARGET.masked ]; then cp $TARGET $OUTPUT_PATH/$TARGET.masked; fi";		# if no repeats are found, RepeatMasker does not create outputfile
	system "rm $OUTPUT_PATH/$TARGET.out $OUTPUT_PATH/$TARGET.cat* ; if [ -f $OUTPUT_PATH/$TARGET.tbl ]; then rm $OUTPUT_PATH/$TARGET.tbl; fi";
...
if (-e "$OUTPUT_PATH/$TARGET_FILE.twinscan") {
    ...
    system("$command > $OUTPUT_PATH/$TARGET_FILE.gtf") == 0
...
sub exit_with_blank_gtf{
    open(OUT, ">$OUTPUT_PATH/$TARGET_FILE.gtf") or die;
    close(OUT);
